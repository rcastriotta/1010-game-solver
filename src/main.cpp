...
int main(int argc, char const* argv[]) {
        ...
        {
            std::string s = argv[1];
            if (s.compare("run-once") == 0) runOnce = true;
        }

        if (runOnce) {
            ai::scoring_function_t scoringFunction = ai::FittingPiecesScoringFunction{ai::fastScoringTable};
            game::Game game{scoringFunction};
            game.printMoves(argv[2], argv[3]);
        } else {
            bool noUi = false;
            ai::scoring_table_t scoringTable;
